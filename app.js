...
const fitButton = getRequiredElement("fitButton");
const zoomInButton = getRequiredElement("zoomInButton");
const zoomOutButton = getRequiredElement("zoomOutButton");
const layerControlsEl = getRequiredElement("layerControls");
const statusEl = getRequiredElement("status");
const edgeInfoEl = document.getElementById("edgeInfo");
const measureInfoEl = getRequiredElement("measureInfo");
const canvas = getRequiredElement("viewer");
...
canvas.addEventListener("pointerdown", (event) => {
    ...
    if (!panByMiddleButton && !panByShiftDrag)
        return;
...
function selectMeasureVertex(event) {
    ...
    if (selectedMeasurePoints.length === 2) {
        ...
        updateMeasurementInfo(`距離: ${measurementDistance.toFixed(3)} (ΔX: ${measurementDeltaX.toFixed(3)}, ΔY: ${measurementDeltaY.toFixed(3)})`);
    }
    else {
...
function updateHoveredEdge(event) {
    if (!drawCommands.length || !bounds || isDragging)
        return;
    const nextHovered = findNearestEdgeFromPointerEvent(event);
    if (hoveredEdge?.layer === nextHovered?.layer &&
...
function updateHoveredVertex(event) {
    ...
    if (hoveredVertex?.x === nextHovered?.x &&
...
function extractDrawData(entities) {
    const commands = [];
    const segments = [];
...
function render() {
    ...
    for (const command of visibleCommands) {
        ...
        if (command.type === "line") {
            ...
            ctx.arc(toCanvasX(command.center.x), toCanvasY(command.center.y), command.radius * scale, 0, Math.PI * 2);
        }
        else if (command.type === "arc") {
            ctx.arc(toCanvasX(command.center.x), toCanvasY(command.center.y), command.radius * scale, -command.end, -command.start, false);
        }
        ctx.stroke();
    }
...
function drawMeasurementOverlay(toCanvasX, toCanvasY) {
    ...
    if (!selectedMeasurePoints.length)
        return;
    ctx.save();
    ctx.lineWidth = 1.5;
